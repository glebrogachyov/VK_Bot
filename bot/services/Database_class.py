...
class Database:
    ...
    def init_table(self, first_load=False):
        try:
            format_ = "%d.%m.%Y"
            current_date = datetime.date(datetime.utcnow() + timedelta(hours=-3)).strftime(format_)
            csv_filename = "Остатки по дисконтным картам на {}.csv".format(current_date)
            db_path = data_folder + csv_filename
